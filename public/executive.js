...
function renderExecutive1(filteredData, rawData) {
  ...
  // All Customer: จำนวนลูกค้าจริงของเดือนนั้นๆ (เหมือน Buyers รายเดือน) แต่ Total Year = ผลรวมบวกกันตรงๆ
  // ของทั้ง 12 เดือน (ไม่ dedup ข้ามเดือน) ต่างจาก Buyers ที่ Total Year เป็นจำนวนลูกค้าไม่ซ้ำกันทั้งปี
  const cumAllCustArr = [];
  ...
  // Total Year ของ All Customer = ผลรวมบวกตรงๆ ของ 12 เดือน (ไม่ dedup) ตามที่ต้องการ
  cumAllCustArr.push(cumAllCustArr.reduce((a, b) => a + b, 0));
