...
async function createUser({ username, name, role, adminName }) {
  const target = (username || '').toLowerCase();
  ...
  await saveUsers(users.concat([newUser]));
  return sanitize(newUser);
}

module.exports = { loadUsers, saveUsers, sanitize, findByUsername, verifyPassword, syncUsers, createUser };
