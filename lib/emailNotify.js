...
// lib/accessRequestStore.js / api/auth/google/[action].js) - an email lands somewhere a Super Admin
// actually checks.
//
...
// wired up by signing up, getting an API key and setting one env var.
//
...
// Only the Super Admins of the organization the request was filed under (entry.orgId) are told.
async function activeSuperAdminEmails(orgId) {
  const users = await userStore.loadUsersByOrg(orgId);
  return users
...
// Only this one event type sends an email.
async function notifyAccessRequestCreated(entry) {
  const toList = await activeSuperAdminEmails(entry.orgId);
  if (toList.length === 0) {
...
// Fire-and-forget - a slow/down email API must
