...
const MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000; // 7-day session timeout - long enough that users only get logged out via the Logout button, not from being idle
...
async function getAuthorizedUser(req) {
  ...
  return {
    ...
    // Always taken from the live user record (never the cookie), so every org-scoped API
    // can trust sessionUser.orgId to decide whose data to read/write.
    orgId: liveUser.orgId
  };
}
